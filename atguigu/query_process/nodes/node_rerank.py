...
class NodeRerank(NodeBase):
    ...
    def cliff_detection(self, rerank_merge_docs):
        ...
        # 如果不-1，那么use_max_topk拿到的刚好是列表的长度
        # 此时遍历会拿到最后一个值，但是要和下一个值比较，但是已经没有下一个值了
        # 此时就会报错
        # 所以要拿到倒数第二个值就要结束
    ...
    def get_rerank_chunks(self, merge_docs, state):
        ...
        # logger.info(json_format(rerank_merge_docs))
        return rerank_merge_docs

    def get_merge_docs(self, state):
        rrf_chunks = state.get("rrf_chunks")
        if not rrf_chunks:
            logger.error("rrf_chunks 为空")
            raise ValueError("rrf_chunks 为空")
        web_search = state.get("web_search_docs")
        merge_docs = rrf_chunks + web_search
        merge_docs = [
            {
